...
class BM25GTERetriever:
    def __init__(self, alpha=0.5):
        """
        Khởi tạo Retriever kết hợp BM25 và GTE-Multilingual
        
        Args:
            alpha: Trọng số kết hợp (0: chỉ BM25, 1: chỉ GTE)
        """
        ...
        # Khởi tạo mô hình GTE-Multilingual
        print("Initializing GTE-Multilingual model...")
        self.embedding_model = SentenceTransformer("Alibaba-NLP/gte-multilingual-base", trust_remote_code=True)
    ...
    def index_corpus(self, corpus):
        """
        Đánh index corpus cho cả BM25 và GTE model
        ...
        # Index cho GTE model
        print("Encoding corpus with GTE-Multilingual...")
        self.corpus_embeddings = self.embedding_model.encode(corpus, return_dense=True, return_sparse=False)
    ...
    def retrieve(self, queries, top_k=10):
        ...
        # Encode queries với GTE model
        print("Encoding queries with GTE-Multilingual...")
        query_embeddings = self.embedding_model.encode(queries, return_dense=True, return_sparse=False)
        ...
        similarity_matrix = np.matmul(query_embeddings, corpus_embeddings_np.T)
...
def evaluate_retriever(data_path, alpha_values=[0, 0.3, 0.5, 0.7, 1.0], k_values=[3, 5, 10, 100]):
    """
    Đánh giá hiệu suất của BM25 kết hợp với GTE-Multilingual
    ...
    for alpha in alpha_values:
        ...
        # Khởi tạo retriever
        retriever = BM25GTERetriever(alpha=alpha)
...
# Hàm chạy thí nghiệm chỉ với BM25 và GTE
def run_bm25_gte_experiment():
    """Chạy thí nghiệm kết hợp BM25 và GTE-Multilingual"""
