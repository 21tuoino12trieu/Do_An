...
model.to(device)
model[0].config.use_flash_attn = False
...
# Mã hóa corpus trước
corpus_embeddings = model.encode(corpus, prompt_name = "retrieval.passage")
...
def calculate_similarities(questions, corpus_embeddings, top_k_values):
    ...
    question_embeddings = model.encode(questions,prompt_name = "retrieval.query")
